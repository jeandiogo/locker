...
// Be aware that locking a file does not prevent other programs from reading/writing to it.
// The locking policy is valid only for programs using this library.
// All methods will throw an exception if an empty filename is given or if the program does not have permission to write to it and to its directory.
// If the file to be locked does not exist, it will be created.
// After locking a file, you still need to open it using the good old fashioned std::fstream.
// Nevertheless, it is a good practice to create a separate lockfile for each file you intend to use;
// (e.g. if you want to exclusively open the file "a.txt", create a lockfile called "a.txt.lock" to use as a mutex for it.)
