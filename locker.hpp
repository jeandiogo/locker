...
// [Notice]
// 
// The locking policy is guaranteed only among programs using this library.
// Thus, locking a file does not prevent other processes from opening it, but it ensures that only one program will get the lock at a time.
// All locking and unlocking functions accept a single filename, a list of filenames, or a vector of filenames.
...
// A process will loose the lock if the lockfile is deleted.
// For this reason, if a file to be unlocked does not exist, an exception will be throw to indicate that a lock may have been lost during the execution, at some point after the lock.
// If you have manually locked a file, do not forget to unlock it.
// If for some reason you have locked a file twice, you have to unlock it twice too.
...
// The locker does not provide thread-safety.
