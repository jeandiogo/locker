...
class locker
{
	...
	template <typename data_t = unsigned char, std::enable_if_t<std::is_arithmetic_v<data_t>> * dummy = nullptr>
	...
	template <typename T, std::enable_if_t<std::is_arithmetic_v<T>> * dummy = nullptr>
	static auto xread(std::string const & filename)
	{
		if(!std::filesystem::exists(filename))
		{
