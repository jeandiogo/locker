...
- **The locker provides process-safety, but not thread-safety.** Once a process has acquired the lock, its threads and future forks will not be stopped by it, nor they will be able to mutually exclude each other by using the lock. Therefore, even though it is ok if a child locks a file after a fork, DO NOT FORK A PROGRAM WHILE IT HAS SOME FILE LOCKED, and use mutexes to synchronize its inner threads.
