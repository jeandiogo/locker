...
Locker is a header-only C++20 class with static member functions to lock files in Linux systems, so they can be accessed exclusively or used as inter-process mutexes. **The locking policy is valid only between programs using this library**, so locking a file does not prevent other processes from accessing it or what it protects. Moreover, **the locker is not thread-safety**, so once a process has acquired the lock, its threads and future forks will not be stopped by it. Avoid forking a program while it has some file locked.
